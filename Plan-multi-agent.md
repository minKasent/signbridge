...
| `feature/video-translate`  | **AGENT B**                   | Dịch ký hiệu từ video file + module analytics + dashboard    |
...
| ------------------ | ---------------------------- | ------------------------------------ | --------------------------- |
| `apps/core`        | chỉ `dictionary/`            | chỉ `analytics/`                     | (tích hợp)                  |
| `apps/web/src/app` | `login/`, `admin/` (tạo mới) | `video/`, `stats/` (đều tạo mới)     | `speak/`                    |
...
## 5. BRANCH B — `feature/video-translate` (AGENT B)

**Mục tiêu**: (1) trang **"Dịch từ video"** — dịch ký hiệu từ FILE VIDEO thay vì webcam,
vì người dùng KHÔNG tự ký hiệu; đây là phương án demo chính: mở clip test của dataset cho
hệ thống dịch trước hội đồng. (2) Module `analytics` lưu số liệu thật + dashboard `/stats`.

> Ý tưởng "chiến dịch thu mẫu" đã bị HỦY (người dùng không quay mẫu). Trang `/collect`
> hiện có GIỮ NGUYÊN, module `dataset` backend KHÔNG đụng tới.

### B1. Trang `/video` — dịch ký hiệu từ file video (tạo mới toàn bộ)

- Chọn file video local (`<input type="file">`, nhận video/mp4 + video/webm) → phát trong
  thẻ `<video>` qua `URL.createObjectURL` — KHÔNG upload lên server, xử lý thuần client.
- Trích landmark từ video đang phát: dùng `createLandmarkers()` + `buildFrame()` từ
  `@/lib/landmarks`. Viết hook riêng `useVideoFilePipeline` đặt TRONG `app/video/`
  (tham khảo vòng lặp `lib/useVisionPipeline.ts` — nguồn là thẻ video file thay vì webcam;
  KHÔNG sửa `useVisionPipeline.ts`). Vẽ khung xương lên canvas đè video; chỉ xử lý khi
  `currentTime` đổi; throttle 25fps theo quy ước.
- Stream qua WebSocket `/ws/translate` y hệt trang `/translate`: batch 5 frame
  `{"type":"frames","frames":[...]}`; nhận `{"type":"gloss"}` và `{"type":"sentence"}`:
  timeline gloss bên phải + khung câu tiếng Việt nổi bật, nút 🔊 đọc lại
  (`speakVietnamese` từ `@/lib/speech`).
- Điều khiển: play/pause; nút "⟲ Dịch lại từ đầu" (seek về 0, gửi `{"type":"flush"}`,
  xóa kết quả cũ); video `onended` → tự gửi flush để chốt câu cuối cùng.
- Video pause/seek thì ngừng gửi frame (không lặp frame khi `currentTime` đứng yên).

### B2. Backend module `analytics` — lưu số liệu thật
...
### B3. Web `/stats` — dashboard (tạo mới)

- Thẻ số tổng + 2 biểu đồ cột **thuần div/CSS** (theo gloss top 10, theo ngày 7 ngày).
- Tự ghi `docs/merge-notes/feature-video-translate.md`: (1) mở công khai
  `GET /api/analytics/**` trong SecurityConfig, (2) link `/video` + `/stats` trên trang chủ.

### B4. Test

- Analytics (Testcontainers, package `analytics`): bắn event `GlossRecognized` bằng
  `ApplicationEventPublisher` trong transaction → poll chờ listener async (tối đa ~10s)
  → GET summary trả đúng số liệu.
- Trang `/video` là client thuần — kiểm bằng tsc/eslint/build; chạy tay ở khâu tích hợp (AGENT C).

### B5. Definition of Done

- [ ] Mở `/video`, chọn một file video có người ký hiệu → khung xương vẽ đè lên video,
      gloss chạy timeline, câu hiện + đọc được (stub trả gloss giả vẫn tính là đạt —
      pipeline thông là mục tiêu)
- [ ] `/translate` chạy một lúc → `/stats` hiện số liệu thật từ DB
...
git worktree add -b feature/video-translate ..\DATN-agent-b main   # cho AGENT B
```
...
3. Làm từng phần (A1→A6 / B1→B5), compile-test-commit theo từng phần.
...
> `feature/video-translate` (đang checkout sẵn trong thư mục này). Thực hiện đầy đủ mục 5
> (B1→B5), tuân thủ tuyệt đối mục 3 và mục 7. KHÔNG chạy dev server/stack. Kết thúc bằng
